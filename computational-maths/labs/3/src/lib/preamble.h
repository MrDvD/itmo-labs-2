...
double eighth_function(vector<double> args) {
  return args[0] - sqrtf32x(1-pow(args[1], 2));
}

double ninth_function(vector<double> args) {
  return args[1] - pow(args[0], 3);
}

double tenth_function(vector<double> args) {
  return args[0] + exp(args[0]) - 2;
}

double eleventh_function(vector<double> args) {
  return args[1] + sin(args[1]) - args[0];
}
...
funcvec_t get_functions(int n) {
  switch (n) {
    ...
    case 4:
      return {eighth_function, ninth_function};
    case 5:
      return {tenth_function, eleventh_function};
    default:
