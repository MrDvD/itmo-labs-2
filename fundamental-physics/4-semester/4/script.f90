...
contains
  ...
  subroutine simulate_interactive(sim, trace_length)
    ...
    allocate(curr_hist%pos(1)%x(16),&
             curr_hist%pos(1)%y(16),&
             curr_hist%pos(1)%ax(16),&
             curr_hist%pos(1)%ay(16),&
             curr_hist%pos(1)%rox(16),&
             curr_hist%pos(1)%roy(16),&
             curr_hist%pos(2)%x(16),&
             curr_hist%pos(2)%y(16),&
             curr_hist%pos(2)%ax(16),&
             curr_hist%pos(2)%ay(16),&
             curr_hist%pos(2)%rox(16),&
             curr_hist%pos(2)%roy(16),&
             curr_hist%t_vals(16))

    curr_hist%capacity = 16
      ...
      select case (ch)
        ...
        case ('d', 'D')
          if (curr_hist%idx == curr_hist%size) then
            ...
            sim%t = sim%t + sim%tau
            call append_history(curr_hist, sim)
          end if
          curr_hist%idx = curr_hist%idx + 1
          sim%t = curr_hist%t_vals(curr_hist%idx)
          call update_bounds_from_trace(curr_hist, trace_length)

        case ('a', 'A')
          if (curr_hist%idx > 1) then
            curr_hist%idx = curr_hist%idx - 1
