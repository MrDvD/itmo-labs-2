\subsection{Цивилизация}

\subsubsection{Описание решения}

В задаче требуется найти кратчайший путь между двумя вершинами. Для этого удобно ввести структуру графа \( G=(V,E) \), где \( v\in V \) --- отдельный квадрат на карте мира, а рёбра между ними строятся по принципу соседства в прямоугольной сетке.

Воспользуемся реализацией алгоритма Дейкстры с использованием очереди с приоритетами. Этот алгоритм рассчитает кратчайшие расстояния до всех вершин от стартовой. Чтобы построить кратчайший путь до конечной вершины в явном виде, нам достаточно хранить сертификат решения --- указатель на прошлую вершину, которая обеспечила релаксацию пути.

В конце работы алгоритма необходимо инвертировать полученную последовательность, и ответ будет получен.

\subsubsection{Асимптотическая сложность}

\textit{Временная сложность решения} --- линейно-логарифмическая \( \mathcal{O}(M\cdot\log N) \). Поиск текущей минимальной вершины с точки зрения расстояния происходит за логарифмическое время, причём таких операций не превышает количество рёбер в графе, связанных со стартовой вершиной --- то есть всех в силу построения графа по прямоугольной сетке. Остальные операции выполняются за константное время.

\textit{Пространственная сложность решения} --- \( \mathcal{O}(N\cdot M) \). В памяти хранится информация с входной матрицы, пускай и преобразованная в несколько иную форму при помощи структур и итераторов. Остальные операции вносят менее существенный вклад в потребление памяти.
