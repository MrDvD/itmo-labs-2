\section{Результаты отображений множества}

В результате работы вышеуказанных скриптов были получены следующие множества:

\begin{center}
  \begin{tabular}{cc}
    \input{script/step_1_tikz} & \input{script/step_2_tikz}\\
    Множество \( P \) & Множество \( P_1=\omega_1(P) \) \\
    \input{script/step_3_tikz} & \input{script/step_4_tikz}\\
    Множество \( P_2=\omega_2(P_1) \) & Множество \( P_3=\omega_3(P_2) \) \\
    \input{script/step_5_tikz} & \input{script/step_6_tikz}\\
    Множество \( P_4=\omega_4(P_3) \) & Множество \( P_5=\omega_5(P_4)=Q \) \\
  \end{tabular}
\end{center}