...
PAIR_COUNT = 10
TuplePoint = Point * 2
ArrayPoint = TuplePoint * PAIR_COUNT
...
def calculate_via_c(arr, c_lib, result):
  c_lib.process_point_arrays.argtypes = [ArrayPoint, ctypes.c_int, ResultList]
  return c_lib.process_point_arrays(arr, PAIR_COUNT, result)

def pretty_print(init_arr, result_arr):
  for i in range(PAIR_COUNT):
    print(f"{i + 1}. {init_arr[i][0]} - {init_arr[i][1]} => {result_arr[i]:.2f}")
