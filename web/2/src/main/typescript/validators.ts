...
export class ParamsFormValidator implements URLParamsValidator {
  public validate(params: URLSearchParams): URLParamsValidationStatus {
      ...
      switch (key) {
        case "X":
          ...
          if (isNaN(X) || X !== intX || Math.abs(X) > 4) {
            errors.push({
              message: "X должен быть целым от -4 до 4 включительно.",
              fieldIdx: 0,
            });
          }
          continue;
        case "Y":
          ...
          if (isNaN(Y) || Math.abs(Y) > 5) {
            errors.push({
              message: "Y должен быть дробным от -5 до 5 включительно.",
              fieldIdx: 1,
            });
          }
          continue;
        case "R":
          ...
          if ([1, 1.5, 2, 2.5, 3].indexOf(R) === -1) {
            errors.push({
              message: "R должен быть одним из значений: 1, 1.5, 2, 2.5, 3.",
              fieldIdx: 2,
            });
          }
          continue;
    ...
    for (let i = 0; i < 3; i++) {
      if (!isParamPresent[i]) {
        errors.push({
          message: `Введите параметр ${paramNames[i]}`,
          fieldIdx: i,
        });
      }
    }
