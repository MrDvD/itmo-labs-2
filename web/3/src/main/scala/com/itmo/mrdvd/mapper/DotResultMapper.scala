...
class DotResultMapper extends Mapper[Dot, DotResult]:
  ...
  override def apply(dot: Dot): Either[DotResult, Error] =
    val inCircle = dot.X >= 0 && dot.Y >= 0 && Math.pow(dot.X, 2) + Math.pow(dot.Y, 2) <= Math.pow(dot.R / 2, 2)
    ...
    Left(DotResult(dot, inCircle || inSquare || inTriangle, ZonedDateTime.now(zone).format(formatter)))